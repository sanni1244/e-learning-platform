    ...
    for(let i = 0; i < mathCourses.length; i++){
        html += `
                <div class="individual">
                    <div>
    <img class="detailsimg" src="${mathCourses[i].img}" alt="">
</div>
<div>
    <div><span class="details">${mathCourses[i].courseCode}</span></div>
    <div><span class="details">${mathCourses[i].name}</span></div>
    <div><span class="details">${mathCourses[i].level}</span></div>
    <div><span class="details"><a href="${mathCourses[i].mylink}">Check Materials</a></span></div>              
</div>
    </div>`
